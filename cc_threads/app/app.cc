...
#include "base_thread/ibase_thread.h"
...
static int ValidateArgs(int argc, char** argv,
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (msg_id == 0) {
            std::cerr
                << "[APP] [ERROR] Message '0' is reserved for internal use."
                << std::endl;
            return -1;
        }
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <msg1> [msg2] [...]"
                  << std::endl;
        return 1;
    }
    ...
    const std::vector<uint32_t>& supported = dispatcher->GetSupportedMsg();

    std::cout << "[APP] [INFO] Dispatcher supports " << supported.size()
              << " unique messages." << std::endl;

    std::vector<uint32_t> valid_msgs_to_send;
    if (ValidateArgs(argc, argv, supported, &valid_msgs_to_send) != 0) {
